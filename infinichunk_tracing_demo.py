...
async def infinichunk_tracing(
    ...
    while iterations < max_iterations:
        # First iteration: full budget; subsequent: only non-carryover part
        params = sampling_params.copy()
        params["max_new_tokens"] = (context_size - carryover_size) if iterations > 0 else context_size
        ...
        # This enforces the carryover and resets the rest of the context.
        prompt_ids = query_ids + response_ids[-carryover_size:]
            ...
            tasks.append(
                asyncio.create_task(
                    infinichunk_tracing(
                        llm=llm,
                        sample=sample,
                        context_size=args.infinichunk_context_size,
                        carryover_size=args.infinichunk_carryover_size,
                        max_iterations=args.infinichunk_iteration_cap,
                    )
                )
            )
...
def main():
    ...
    parser.add_argument(
        "--infinichunk_context_size",
        type=int,
        default=8192,
        help="INFINICHUNK context size (C)",
    )
    parser.add_argument(
        "--infinichunk_carryover_size",
        type=int,
        default=4096,
        help="INFINICHUNK carryover size (m)",
    )
    parser.add_argument(
        "--infinichunk_iteration_cap",
        type=int,
        default=5,
        help="INFINICHUNK iteration cap (I)",
    )
    parser.add_argument(
        "--debug",
        type=bool,
    ...
    #   C + (I-1) * (C - m)
    # where C = infinichunk_context_size, m = carryover_size.
    effective_token_budget = args.infinichunk_context_size + (args.infinichunk_iteration_cap - 1) * (
        args.infinichunk_context_size - args.infinichunk_carryover_size
    )
    ...
    # Base prompt budget (outside Infinichunk chunking). Kept as a fixed cap here.
    max_prompt_length = 2048

    # Initialize sglang Engine.
    # context_length accounts for the base prompt plus the per-chunk Infinichunk budget.
    llm = sgl.Engine(
        ...
        context_length=max_prompt_length + args.infinichunk_context_size,
