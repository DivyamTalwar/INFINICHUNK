...
def dispatch_dp_compute_data_proto_with_indices(worker_group, *args, **kwargs):
    worker_indices = kwargs.pop("VERL_DISPATCH_INDICES", None)
    ...
    assert isinstance(worker_group, WorkerGroup)
    assert isinstance(worker_indices, list)
    assert len(worker_indices) == worker_group.world_size
    assert all(isinstance(indices, torch.Tensor | np.ndarray | list) for indices in worker_indices)

    splitted_args = []
    for arg in args:
        assert isinstance(arg, DataProto | DataProtoFuture)
        splitted_args.append([arg.select_idxs(indices) for indices in worker_indices])

    splitted_kwargs = {}
    for key, val in kwargs.items():
        assert isinstance(val, DataProto | DataProtoFuture)
        splitted_kwargs[key] = [val.select_idxs(indices) for indices in worker_indices]
