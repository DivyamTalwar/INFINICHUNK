...
def ensure_divisible(data: "DataProto", size_divisor: int, mode: Literal["copy", "delete"] = "delete") -> "DataProto":
    """Ensure the DataProto is divisible by size_divisor by padding if necessary.

    Args:
        data: (DataProto): the DataProto to check
        size_divisor (int): the size divisor
    ...
    if mode == "delete":
        ...
        keep_indices = np.setdiff1d(np.arange(bs), remove_indices)
        keep_batch = data.select_idxs(keep_indices)

        adjusted_batch = keep_batch
    elif mode == "copy":
