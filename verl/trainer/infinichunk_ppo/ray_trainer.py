...
"""
InfinichunkPPO Trainer with Ray-based single controller.
...
@register_trainer("infinichunk_ppo")
class RayInfinichunkPPOTrainer(RayTreetunePPOTrainer):
    def init_workers(self):
        """
        Customized init_workers for Infinichunk PPO.
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # patch agent_name to infinichunk
                batch.non_tensor_batch["agent_name"] = np.array(["infinichunk_agent"] * len(batch), dtype=object)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        assert not self.config.reward_model.launch_reward_fn_async, (
                            "Async reward is not supported for infinichunk"
                        )
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.config.algorithm.use_kl_in_reward:
                            raise NotImplementedError("KL penalty is not supported for infinichunk")
                    ...
                    metrics.update(self._compute_infinichunk_metrics(batch))
                    ...
                    # convert intermediate infinichunk turns into normal episodes
                    ...
                    with marked_timer("flatten_intermediate_turns", timing_raw, color="purple"):
                        ...
                        metrics.update({"infinichunk/batch_size_after_flatten": len(batch)})
                    ...
                    if should_save_rollout:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            self._dump_infinichunk_generations(batch=batch, dump_path=rollout_data_dir)
                    ...
                    if self.config.algorithm.infinichunk.fixed_num_optim_steps > 0:
                        batch.meta_info["fixed_num_optim_steps"] = self.config.algorithm.infinichunk.fixed_num_optim_steps

                    loss_agg_mode = self.config.actor_rollout_ref.actor.loss_agg_mode
                    if loss_agg_mode == "seq-mean-token-norm-trace-length":
                        if self.config.algorithm.infinichunk.get("use_flat_batch_correction_ratio", False):
                            batch.batch["trace_lengths"] = batch.batch["trace_lengths"].float() * (
                                before_flatten_bsz / len(batch)
                            )
                            metrics.update({"infinichunk/flat_batch_correction_ratio": len(batch) / before_flatten_bsz})
                    ...
                    # compute values
                    if self.use_critic:
                        raise NotImplementedError("Critic is not supported for infinichunk")
    ...
    def _flatten_intermediate_turns(self, batch: DataProto) -> DataProto:
        """Convert batch with infinichunk turns into a list of state-action rows (normal episodes)"""
    ...
    def _ensure_divisible_by_dp_size_and_training_batch_size(self, batch: DataProto, mode: str = "delete") -> DataProto:
        ...
        if self.config.algorithm.infinichunk.fixed_num_optim_steps > 0:
            divisor = np.lcm.reduce(np.array([dp_size, self.config.algorithm.infinichunk.fixed_num_optim_steps])).item()
        else:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # patch agent_name to infinichunk
            test_batch.non_tensor_batch["agent_name"] = np.array(["infinichunk_agent"] * len(test_batch), dtype=object)
            ...
            if "__num_turns__" in test_batch.non_tensor_batch:
                ...
                reward_extra_infos_dict["infinichunk_num_turns"].extend(
    ...
    def _compute_infinichunk_metrics(self, unflattened_batch: DataProto) -> dict[str, float]:
        """compute per-trajectory metrics
        Args:
        ...
        metrics = {
            # Scores
            "infinichunk/trace_scores/mean": np.mean(scores),
            "infinichunk/trace_scores/std": np.std(scores),
            "infinichunk/trace_scores/max": np.max(scores),
            "infinichunk/trace_scores/min": np.min(scores),
            "infinichunk/trace_scores/dist": scores,
            # Num turns
            "infinichunk/trace_num_turns/mean": np.mean(num_turns),
            "infinichunk/trace_num_turns/std": np.std(num_turns),
            "infinichunk/trace_num_turns/max": np.max(num_turns),
            "infinichunk/trace_num_turns/min": np.min(num_turns),
            "infinichunk/trace_num_turns/dist": num_turns,
            # Lengths
            "infinichunk/trace_lengths/mean": np.mean(lengths),
            "infinichunk/trace_lengths/std": np.std(lengths),
            "infinichunk/trace_lengths/max": np.max(lengths),
            "infinichunk/trace_lengths/min": np.min(lengths),
            "infinichunk/trace_lengths/dist": lengths,
            # Has EOS
            "infinichunk/trace_has_eos/mean": np.mean(has_eos),
            "infinichunk/trace_has_eos/std": np.std(has_eos),
        }
    ...
    def _dump_infinichunk_generations(self, batch: DataProto, dump_path: str):
        """Dump rollout/validation samples as JSONL."""
        os.makedirs(dump_path + "_infinichunk", exist_ok=True)
        filename = os.path.join(dump_path + "_infinichunk", f"{self.global_steps}.jsonl")
        ...
        template = (
            ("#" * 30 + "\n# INFINICHUNK TURN {del_turn_idx:02d}\n" + "#" * 30 + "\n")
            + "# input:\n"
            + ("-" * 30 + "\n")
            + "{input}\n"
            + ("#" * 30 + "\n# INFINICHUNK TURN {del_turn_idx:02d}\n" + "#" * 30 + "\n")
        ...
        for traj_id in traj_indices:
            ...
            base_data["num_infinichunk_turns"].append(item.non_tensor_batch["__num_turns__"].item() - 1)
        ...
        log_kwargs = {
            **base_data,
            "idx": list(range(n)),
            "_prefix": "train/generations_infinichunk",
            "_columns": ["idx", "input", "output", "gts", "score", "num_infinichunk_turns", "trace_length", "advantage"],
        }
