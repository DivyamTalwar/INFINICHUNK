...
def compute_data_metrics_with_distribution(
    batch: DataProto, use_critic: bool = True, compute_score_metrics: bool = True
) -> dict[str, Any]:
    """
    Computes various metrics from a batch of data for PPO training.
    ...
    score_max = torch.max(non_aborted_sequence_score).detach().item()
    score_min = torch.min(non_aborted_sequence_score).detach().item()
    score_dist = non_aborted_sequence_score.detach().cpu().numpy()

    reward_mean = torch.mean(non_aborted_sequence_reward).detach().item()
    reward_max = torch.max(non_aborted_sequence_reward).detach().item()
    reward_min = torch.min(non_aborted_sequence_reward).detach().item()
    reward_dist = non_aborted_sequence_reward.detach().cpu().numpy()

    valid_adv = torch.masked_select(advantages, response_mask)
    valid_adv_dist = valid_adv.detach().cpu().numpy()
    valid_returns = torch.masked_select(returns, response_mask)
    valid_returns_dist = valid_returns.detach().cpu().numpy()

    if use_critic:
        values = batch.batch["values"]
        valid_values = torch.masked_select(values, response_mask)
        valid_values_dist = valid_values.detach().cpu().numpy()
        return_diff_var = torch.var(valid_returns - valid_values)
        return_var = torch.var(valid_returns)
    ...
    if non_aborted_response_length.numel() > 0:
        ...
        non_aborted_response_length_max = torch.max(non_aborted_response_length).detach().item()
        non_aborted_response_length_min = torch.min(non_aborted_response_length).detach().item()
        non_aborted_response_length_dist = non_aborted_response_length.detach().cpu().numpy()
        non_aborted_response_length_clip_ratio = (
            torch.mean(torch.eq(non_aborted_response_length, max_response_length).float()).detach().item()
        )
    else:
    ...
    metrics = {
        ...
        "critic/score/max": score_max,
        "critic/score/min": score_min,
        "critic/score/dist": score_dist,
        # reward
        "critic/rewards/mean": reward_mean,
        "critic/rewards/max": reward_max,
        "critic/rewards/min": reward_min,
        "critic/rewards/dist": reward_dist,
        # adv
        "critic/advantages/mean": torch.mean(valid_adv).detach().item(),
        "critic/advantages/max": torch.max(valid_adv).detach().item(),
        "critic/advantages/min": torch.min(valid_adv).detach().item(),
        "critic/advantages/dist": valid_adv_dist,
        # returns
        "critic/returns/mean": torch.mean(valid_returns).detach().item(),
        "critic/returns/max": torch.max(valid_returns).detach().item(),
        "critic/returns/min": torch.min(valid_returns).detach().item(),
        "critic/returns/dist": valid_returns_dist,
        **(
            {
                # values
                "critic/values/mean": torch.mean(valid_values).detach().item(),
                "critic/values/max": torch.max(valid_values).detach().item(),
                "critic/values/min": torch.min(valid_values).detach().item(),
                "critic/values/dist": valid_values_dist,
                # vf explained var
                "critic/vf_explained_var": (1.0 - return_diff_var / (return_var + 1e-5)).detach().item(),
            }
            if use_critic
        ...
        .detach()
        .item(),
        "response_length/dist": response_length.detach().cpu().numpy(),
        # response length (non-aborted only)
        # These statistics exclude aborted samples to avoid skew from zeros
        "response_length_non_aborted/mean": non_aborted_response_length_mean,
        "response_length_non_aborted/max": non_aborted_response_length_max,
        "response_length_non_aborted/min": non_aborted_response_length_min,
        "response_length_non_aborted/clip_ratio": non_aborted_response_length_clip_ratio,
        "response_length_non_aborted/dist": non_aborted_response_length_dist,
        # aborted ratio
        # Fraction of samples whose response length is zero
        "response/aborted_ratio": aborted_ratio,
        # prompt length
    ...
    if compute_score_metrics:
        score_metrics_keys = [key for key in batch.non_tensor_batch.keys() if key.startswith("score_metrics/")]
...
def compute_per_scores_metrics(
    scores: np.ndarray,
    ...
    """
    assert all(len(scores) == len(metric) for metric in other_metrics.values()), "All metrics must have the same length"

    # Separate trajectories by positive and non-positive scores
    positive_mask = scores > 0
    non_positive_mask = scores <= 0

    def compute_stats(arr):
        if arr.shape[0] > 0:
            return {"mean": np.mean(arr), "std": np.std(arr), "max": np.max(arr), "min": np.min(arr), "dist": arr}
        else:
            return {"mean": 0.0, "std": 0.0, "max": 0.0, "min": 0.0, "dist": []}

    metrics = {}

    for metric_name, metric_arr in other_metrics.items():
        metrics.update({f"positive_score/{metric_name}": compute_stats(metric_arr[positive_mask])})
        metrics.update({f"non_positive_score/{metric_name}": compute_stats(metric_arr[non_positive_mask])})
...
def compute_group_metrics(
    ...
    """
    Compute rollout group metrics for each episode.

    Args:
        batch: A DataProto object containing batch data with episode information.

    Returns:
        A dictionary containing:
            - group_metrics/zero_std_ratio: Ratio of prompts with zero standard deviation in rollout group
    """

    if group_key not in batch.non_tensor_batch:
        return {}

    scores = batch.batch[score_key].sum(-1).tolist()

    uid_to_scores = defaultdict(list)
    for uid, score in zip(batch.non_tensor_batch[group_key], scores, strict=True):
        uid_to_scores[uid].append(score)

    scores_std = np.array([np.std(scores) for scores in uid_to_scores.values()])

    return {
        "group_metrics/zero_std_ratio": np.mean(scores_std < 1e-6),
    }


def compute_has_eos(batch: DataProto, eos_token_id: int, return_numpy: bool = True) -> np.ndarray | torch.Tensor:
    from verl.trainer.ppo.ray_trainer import compute_response_mask

    response_mask = batch.batch.get("response_mask", compute_response_mask(batch))
    response_ids = batch.batch["responses"]
    has_eos = (response_ids == eos_token_id) & response_mask
    has_eos = has_eos.sum(-1)
    assert torch.all(has_eos <= 1), "Each trajectory should have at most one EOS token"
    return has_eos.cpu().numpy() if return_numpy else has_eos


def compute_eos_metrics(batch: DataProto, eos_token_id: int) -> dict[str, Any]:
    has_eos = compute_has_eos(batch, eos_token_id)
    return {"has_eos/mean": np.mean(has_eos), "has_eos/std": np.std(has_eos)}
...
def compute_timing_and_throughput_metrics(batch: DataProto, timing_raw: dict[str, float]) -> dict[str, Any]:
    """
    Computes timing metrics for different processing stages in PPO training.
    ...
    return {
        ...
        **{
            f"e2e_throughput/{name}": num_tokens_of_section[name] / timing_raw[name]
            for name in set(num_tokens_of_section.keys()) & set(timing_raw.keys())
        },
    }
