...
AGG_MODE_NEEDS_TRACE_LENGTH = ["seq-mean-token-norm-trace-length"]


def agg_loss_with_trace_lengths(
    loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str, trace_lengths: Optional[torch.Tensor] = None
) -> torch.Tensor:
    if loss_agg_mode not in AGG_MODE_NEEDS_TRACE_LENGTH:
        return agg_loss(loss_mat=loss_mat, loss_mask=loss_mask, loss_agg_mode=loss_agg_mode)

    assert trace_lengths is not None, "Trace lengths are required for this aggregation mode"

    if loss_agg_mode == "seq-mean-token-norm-trace-length":
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1) / trace_lengths  # token-mean
        loss = torch.mean(seq_losses)  # seq-mean
    else:
        raise ValueError(f"Invalid loss_agg_mode: {loss_agg_mode}")
...
def compute_policy_loss_vanill_no_aggr(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    if config.tis_imp_ratio_cap > 0:
        # Apply truncated importance sampling -> https://fengyao.notion.site/off-policy-rl
        assert rollout_log_probs is not None, "rollout_log_probs is required for TIS"
...
@register_policy_loss("vanilla_with_trace_lengths")
def compute_policy_loss_with_trace_lengths(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    loss_agg_mode: str = "token-mean",
    trace_lengths: Optional[torch.Tensor] = None,
    rollout_log_probs: Optional[torch.Tensor] = None,
    config: Optional[DictConfig | AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    We only modify the behavior of loss computation if `loss_agg_mode` is in `AGG_MODE_NEEDS_TRACE_LENGTH`.
    ...
    Args:
        ...
        response_mask (torch.Tensor):
            Mask indicating which tokens to include in the loss, shape (batch_size, response_length).
        trace_lengths (torch.Tensor, optional):
            Trace lengths for each action, shape (batch_size).
        loss_agg_mode (str, optional):
            Aggregation mode for `agg_loss`. Defaults to "token-mean".
        rollout_log_probs (torch.Tensor, optional):
            Log-probabilities of actions under the rollout policy, shape (batch_size, response_length).
    """
    if loss_agg_mode not in AGG_MODE_NEEDS_TRACE_LENGTH and rollout_log_probs is None:
    ...
    pg_losses, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss_vanill_no_aggr(
        old_log_prob=old_log_prob,
        log_prob=log_prob,
        advantages=advantages,
        response_mask=response_mask,
    ...
    pg_loss = agg_loss_with_trace_lengths(
        loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, trace_lengths=trace_lengths
    )
    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower
