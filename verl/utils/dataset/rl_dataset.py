...
class RLHFDataset(Dataset):
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            overlong_prompt_length = self.config.get("overlong_prompt_length", self.max_prompt_length)
            dataframe = dataframe.filter(
                lambda doc: doc2len(doc) <= overlong_prompt_length,
                num_proc=self.num_workers,
                desc=f"Filtering prompts longer than {overlong_prompt_length} tokens",
            )
