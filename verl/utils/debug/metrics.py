...
def calculate_rollout_probs_diff_metrics(data: DataProto) -> dict:
    """
    calculate rollout vs actor logprobs diff, for debugging purpose
    ...
    Returns:
        dict: metrics
            "training/rollout_probs_diff/dist": distribution of logprob diff of rollout vs. actor
            "training/rollout_probs_diff/max": max value of logprob diff of rollout vs. actor
            "training/rollout_probs_diff/mean": mean value of logprob diff of rollout vs. actor
            "training/rollout_probs_diff/std": std value of logprob diff of rollout vs. actor
            "training/rollout_probs_diff/pearson_corr": logprob's pearson corrcoef of rollout vs. actor, reference to https://arxiv.org/pdf/2506.13585
    """
    ...
    imp_ratio = torch.exp(actor_old_log_probs - rollout_old_log_probs)
    imp_ratio = torch.masked_select(imp_ratio, response_mask_bool)

    return {
        "training/rollout_probs_diff/dist": rollout_probs_diff.cpu().numpy(),
        "training/rollout_probs_diff/max": torch.max(rollout_probs_diff).detach().item(),
        "training/rollout_probs_diff/mean": torch.mean(rollout_probs_diff).detach().item(),
        "training/rollout_probs_diff/std": torch.std(rollout_probs_diff).detach().item(),
        "training/rollout_probs_diff/pearson_corr": pearson_corrcoef,
        "training/imp_ratio/dist": imp_ratio.cpu().numpy(),
        "training/imp_ratio/max": torch.max(imp_ratio).detach().item(),
        "training/imp_ratio/min": torch.min(imp_ratio).detach().item(),
        "training/imp_ratio/mean": torch.mean(imp_ratio).detach().item(),
        "training/imp_ratio/std": torch.std(imp_ratio).detach().item(),
    }
