...
class ValidationGenerationsLogger:
    ...
    def log_in_single_table(self, loggers, samples, step):
        if "wandb" in loggers:
            import wandb

            self._log_generations_to_wandb_single_table(samples, step, wandb)
    ...
    def _log_generations_to_wandb_single_table(self, samples: dict[str, list], step, wandb):
        """Log samples to wandb as a table in a custom format; one table per step"""

        # Create column names for all samples
        columns = samples.pop("_columns", sorted(samples.keys()))
        prefix = samples.pop("_prefix", "val/generations")

        table = wandb.Table(columns=columns)

        # Add new row with all data
        n_samples = len(samples[columns[0]])
        for i in range(n_samples):
            row_data = [samples[col][i] for col in columns]
            table.add_data(*row_data)

        # Update reference and log
        wandb.log({f"{prefix}/step_{step:06d}": table}, step=step)
