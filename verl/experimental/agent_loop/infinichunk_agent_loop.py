...
class _InfinichunkAgentLoopOutput(AgentLoopOutput):
    extra_fields: dict[str, Any] = field(default_factory=dict)


@register("infinichunk_agent")
class InfinichunkAgentLoop(AgentLoopBase):
    @classmethod
    def init_class(cls, config, tokenizer, processor, **kwargs):
        if cls._class_initialized:
            return
        cls._class_initialized = True
        print("Performing class-level InfinichunkAgentLoop initialization")
        ...
        cls.keep_head = config.algorithm.infinichunk.keep_head
        cls.keep_tail = config.algorithm.infinichunk.keep_tail
        cls.intermediate_max_new_tokens = config.algorithm.infinichunk.intermediate_max_new_tokens

        cls.trimmer = None
        if config.algorithm.infinichunk.use_trimmer_class:
            cls.trimmer = get_trimmer_cls(config.algorithm.infinichunk.trimmer_name)(
                keep_first=cls.keep_head,
                keep_last=cls.keep_tail,
                tokenizer=tokenizer,
                **config.algorithm.infinichunk.trimmer_kwargs,
            )
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        output = _InfinichunkAgentLoopOutput(
