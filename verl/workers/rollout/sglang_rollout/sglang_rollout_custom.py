...
class CustomSGLangRollout(SGLangRollout):
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        self._enforce_min_new_tokens = engine_kwargs.pop("enforce_min_new_tokens", False)
        if self._enforce_min_new_tokens:
            logger.warning(
                "Enforcing min_new_tokens to be equal to max_new_tokens; It should be only used for debugging purposes"
            )
        ...
        if first_rank_in_node:
            ...
            self._engine = AsyncEngine(
                ...
                # inconsistency issue.
                skip_tokenizer_init=self.config.mode == "async" and not self._enforce_min_new_tokens,
                **engine_kwargs,
            )
        else:
            self._engine = None
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._enforce_min_new_tokens:
            request_sampling_params["min_new_tokens"] = request_sampling_params["max_new_tokens"]
    ...
    async def _handle_engine_generate(
        ...
        if self._enforce_min_new_tokens:
            kwargs["min_new_tokens"] = kwargs["max_new_tokens"]
